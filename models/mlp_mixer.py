...
class MLPMixerV2_AddLayer(nn.Module):
    def __init__(self, image_size=224, patch_size=32, in_channels=3, num_classes=5,
                 embed_dim=512, depth=9, token_mlp_dim=256, channel_mlp_dim=2048, dropout=0.2):
        super().__init__()
        assert image_size % patch_size == 0, 'Image dimensions must be divisible by the patch size.'
        num_patches = (image_size // patch_size) ** 2
        patch_dim = in_channels * patch_size * patch_size
...
class MixerLayerWithBatchNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + self.channel_mixing(self.norm2(x.transpose(1, 2)).transpose(1, 2))
        return x

class MLPMixerV2BatchNorm(nn.Module):
    def __init__(self, image_size=224, patch_size=16, in_channels=3, num_classes=5,
        ...
        self.mixer_layers = nn.Sequential(*[
            MixerLayerWithBatchNorm(num_patches, embed_dim, token_mlp_dim, channel_mlp_dim, dropout)
            for _ in range(depth)
        ])
