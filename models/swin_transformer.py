...
class SwinIJICTransformer_AddStage(nn.Module):
    def __init__(self, num_classes=5):
        super().__init__()
        self.num_classes = num_classes
        self.embed_dim = 32
        ...
        self.patch_merge3 = PatchMerging((14, 14), 4 * self.embed_dim)

        self.stage4 = nn.Sequential(
            SwinBlock(8 * self.embed_dim, (7, 7), num_heads=8, window_size=7, shift_size=0)
        )

        self.norm = nn.LayerNorm(8 * self.embed_dim)
        self.head = nn.Linear(8 * self.embed_dim, num_classes)
...
class SwinIJICTransformer_AddExtraBlocks(nn.Module):
    def __init__(self, num_classes=5):
        super().__init__()
        self.num_classes = num_classes
        self.embed_dim = 32
        ...
        self.stage1 = nn.Sequential(
            SwinBlock(self.embed_dim, (56, 56), num_heads=2),
            SwinBlock(self.embed_dim, (56, 56), num_heads=2)
        )
        self.patch_merge1 = PatchMerging((56, 56), self.embed_dim)

        self.stage2 = nn.Sequential(
            SwinBlock(2 * self.embed_dim, (28, 28), num_heads=4),
            SwinBlock(2 * self.embed_dim, (28, 28), num_heads=4)
        )
        self.patch_merge2 = PatchMerging((28, 28), 2 * self.embed_dim)

        self.stage3 = nn.Sequential(
            SwinBlock(4 * self.embed_dim, (14, 14), num_heads=8),
            SwinBlock(4 * self.embed_dim, (14, 14), num_heads=8)
        )
        self.patch_merge3 = PatchMerging((14, 14), 4 * self.embed_dim)

        self.stage4 = nn.Sequential(
            SwinBlock(8 * self.embed_dim, (7, 7), num_heads=8),
            SwinBlock(8 * self.embed_dim, (7, 7), num_heads=8)
        )
