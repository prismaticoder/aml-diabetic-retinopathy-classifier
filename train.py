...
def train_epoch(model, loader, criterion, optimizer, device, progress_bar, task_id):
    ...
    for images, labels in loader:
        ...
        optimizer.zero_grad()
        outputs = model(images)  # Should output logits, shape: [batch_size, num_classes]

        # Debugging: print the shapes of the outputs and labels
        print(f"Train - Outputs shape: {outputs.shape}, Labels shape: {labels.shape}")
        
        
        # Ensure labels are integers, not one-hot encoded
        loss = criterion(outputs, labels.long())  # CrossEntropyLoss expects labels to be integers

        loss.backward()
        optimizer.step()

        total_loss += loss.item()
        _, preds = torch.max(outputs, 1)  # Get the predicted class by selecting the max logit
...
def validate(model, loader, criterion, device):
    ...
    with torch.no_grad():
        for images, labels in loader:
            ...
            # Debugging: print the shapes of the outputs and labels
            print(f"Val - Outputs shape: {outputs.shape}, Labels shape: {labels.shape}")
            
            # Ensure labels are integers, not one-hot encoded
            loss = criterion(outputs, labels)  # CrossEntropyLoss expects labels to be integers
            total_loss += loss.item()

            _, pred = torch.max(outputs, 1)  # Get the predicted class by selecting the max logit
...
if __name__ == "__main__":
    ...
    # Get model and input size based on model name
    model, input_size = get_model(args.model.lower(), weights=args.weights)

    # Move model to the device (GPU or CPU)
    model = model.to(device)

    # Update image resize based on model's input size requirement
    train_transform = A.Compose([
        A.Resize(input_size, input_size),  # Use dynamic input_size
    ...
    # Get data loaders
    train_loader, val_loader = get_data_loaders(args.csv_root_dir, args.img_dir, args.batch_size, input_size=input_size)  # Pass input_size
