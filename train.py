...
def train_epoch(model, loader, criterion, optimizer, device, scaler, progress, task_id):
    model.train()
...
if __name__ == "__main__":
    ...
    train_loader, val_loader = get_data_loaders(args.csv_root_dir, args.img_dir, args.batch_size, num_workers=args.num_workers, profile=args.augmentation_profile)
    model_dir = f"output/{args.model}_opt{args.optim}_lr{args.learning_rate}_bs{args.batch_size}_loss{args.loss}_aug{args.augmentation_profile}"
    ...
    with Progress(SpinnerColumn(), TextColumn("[blue]{task.description}"), BarColumn(),
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            train_epoch_loss, train_epoch_acc = train_epoch(model, train_loader, criterion, optimizer, device, scaler, progress, task_id)
            ...
            if args.early_stopping:
                if val_epoch_loss < best_val_loss:
                    ...
                    if patience_counter >= patience:
                        console.print(f"❌ [red]Early stopping triggered at epoch {epoch}[/red]")
